...
// 1) Initialize controls with async wrapper to avoid top-level await
(async () => {
  // Preload all six icons as Fabric‐compatible images
  const [
    deleteImg,
    layerImg,
    cloneImg,
  ...
  // 2) Helper to construct a Control that draws your SVG icon
  ...
  // 3) Patch Textbox.prototype.controls once at module load
  ...
  // 4) Disable objectCaching so controls always redraw
  Textbox.prototype.objectCaching = false;
})();

// Export nothing—just importing this file in your app will install the controls.
// In your React component, use only these Fabric classes:
